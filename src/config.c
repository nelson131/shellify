...
int config_load(Config** config) {
    ...
    while (fgets(line, CONFIG_LINE_SIZE, file) != NULL) {
        ...
        if (sscanf(line, "move=%c", &temp->keys.move) == 1) continue;
...
int config_save(Config* config) {
    ...
    fprintf(file,
            ...
            config->keys.pause, config->keys.edit, config->keys.move,
            config->keys.inc, config->keys.dec, config->keys.skip_f,
            config->keys.skip_b, config->keys.add, config->keys.remove,
            config->keys.song, config->keys.playlist, config->keys.shuffle,
...
void config_default(Config* config) {
    ...
    config->keys.move = CONFIG_DEF_MOVE;
